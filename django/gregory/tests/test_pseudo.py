	...
	def test_csv_path_creation(self):
		...
		with tempfile.TemporaryDirectory() as tmp_dir:
			...
			with patch("gregory.ml.pseudo.datetime") as mock_datetime:
				...
				expected_path_with_prefix = (
					Path(tmp_dir) / f"test_{fixed_timestamp}.csv"
				)
...
class TestGeneratePseudoLabels:
	...
	def test_pseudo_labeling_loop(self, mock_get_trainer):
		...
		assert set(pseudo_rows["pseudo_iteration"]) == {
			1,
			1,
			2,
		}  # 2 from first iter, 1 from second

		# Check that all pseudo-labeled rows have confidence values
		assert all(
			not pd.isna(row["pseudo_confidence"]) for _, row in pseudo_rows.iterrows()
		)
...
class TestLoadAndFilterPseudoLabels:
	...
	def test_load_and_filter(self):
		...
		with tempfile.TemporaryDirectory() as tmp_dir:
			...
			filtered_high_conf = load_and_filter_pseudo_labels(
				file_path, min_confidence=0.9
			)
			...
			filtered_early_iter = load_and_filter_pseudo_labels(
				file_path, max_iteration=1
			)
			...
			only_pseudo = load_and_filter_pseudo_labels(
				file_path, include_original=False
			)
	...
	def test_error_handling(self):
		...
		with tempfile.TemporaryDirectory() as tmp_dir:
			...
			invalid_df = pd.DataFrame({"text": ["text1", "text2"], "relevant": [1, 0]})
			...
			with pytest.raises(
				ValueError, match="does not contain pseudo-labeling information"
			):
