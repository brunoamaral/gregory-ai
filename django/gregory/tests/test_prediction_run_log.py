...
class PredictionRunLogTestCase(TestCase):
	def setUp(self):
		...
		self.subject2 = Subject.objects.create(
			subject_name="Cardiology", team=self.team, subject_slug="cardiology"
		)
	...
	def test_get_latest_run(self):
		...
		latest_train = PredictionRunLog.get_latest_run(
			self.team, self.subject1, run_type="train"
		)
		self.assertEqual(latest_train, self.train_log)

		# Get latest prediction run for subject1
		latest_predict = PredictionRunLog.get_latest_run(
			self.team, self.subject1, run_type="predict"
		)
		...
		latest_subject2_after = PredictionRunLog.get_latest_run(
			self.team, self.subject2
		)
