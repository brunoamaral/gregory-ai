	...
	def setUp(self):
		...
		def mock_bert_call(inputs, attention_mask=None, **kwargs):
			...
			sequence_output = tf.constant(
				np.ones((batch_size, 10, 768)), dtype=tf.float32
			)
		...
		self.mock_dense_instance.side_effect = lambda x: tf.constant(
			np.random.rand(2, 2), dtype=tf.float32
		)

		# Make the Dropout layer callable and return a tensor
		self.mock_dropout_instance.side_effect = lambda x: tf.constant(
			np.random.rand(2, 2), dtype=tf.float32
		)

		# Setup the model instance with inputs and outputs attributes
		self.mock_model_instance.inputs = [MagicMock(), MagicMock()]
		self.mock_model_instance.outputs = [
			tf.constant(np.random.rand(2, 2), dtype=tf.float32)
		]
	...
	def test_evaluate(self):
		...
		with (
			...
			metrics = self.trainer.evaluate(
				test_texts=["Test 1", "Test 2"], test_labels=[1, 0], threshold=0.5
			)
	...
	def test_predict(self):
		...
		with patch.object(self.trainer.model, "predict") as mock_predict:
			...
			predictions, probabilities = self.trainer.predict(
				texts=["Text 1", "Text 2"], threshold=0.5
			)
