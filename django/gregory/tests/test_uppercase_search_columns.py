...
class UppercaseSearchColumnsTestCase(TestCase):
	...
	def test_uppercase_columns_exist(self):
		...
		with connection.cursor() as cursor:
			cursor.execute(
				"SELECT column_name FROM information_schema.columns WHERE table_name='articles' AND column_name IN ('utitle', 'usummary')"
			)
			...
			cursor.execute(
				"SELECT column_name FROM information_schema.columns WHERE table_name='trials' AND column_name IN ('utitle', 'usummary')"
			)
	...
	def test_article_search_case_insensitive(self):
		...
		old_results = Articles.objects.filter(title__icontains=search_term).union(
			Articles.objects.filter(summary__icontains=search_term)
		)

		# New way using uppercase columns
		upper_term = search_term.upper()
		new_results = Articles.objects.filter(utitle__contains=upper_term).union(
	...
	def test_trial_search_case_insensitive(self):
		...
		# Old way (for comparison)
		old_results = Trials.objects.filter(title__icontains=search_term).union(
			Trials.objects.filter(summary__icontains=search_term)
		)

		# New way using uppercase columns
		upper_term = search_term.upper()
		new_results = Trials.objects.filter(utitle__contains=upper_term).union(
	...
	def test_mixed_case_search(self):
		...
		for term in search_terms:
			...
			results = Articles.objects.filter(utitle__contains=upper_term).union(
				Articles.objects.filter(usummary__contains=upper_term)
			)
			...
			expected_ids = {self.article1.article_id, self.article2.article_id}
			self.assertEqual(
				result_ids, expected_ids, f"Failed for search term: {term}"
			)
	...
	def test_empty_search_handling(self):
		...
		self.assertEqual(
			results.count(), Articles.objects.count()
