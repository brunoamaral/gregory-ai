...
class VersioningTestCase(TestCase):
	...
	def test_make_version_path_creates_directory(self):
		...
		expected_path = (
			Path(self.temp_dir) / "team1" / "covid" / "pubmed_bert" / "20250518"
		)
	...
	def test_make_version_path_increments_suffix_when_exists(self):
		...
		expected_second_path = (
			Path(self.temp_dir) / "team2" / "diabetes" / "lstm" / "20250518_2"
		)
		...
		expected_third_path = (
			Path(self.temp_dir) / "team2" / "diabetes" / "lstm" / "20250518_3"
		)
	...
	def test_make_version_path_handles_different_algorithms(self):
		...
		self.assertEqual(
			path1, Path(self.temp_dir) / "team3" / "cancer" / "pubmed_bert" / "20250518"
		)
		self.assertEqual(
			path2, Path(self.temp_dir) / "team3" / "cancer" / "lgbm_tfidf" / "20250518"
		)
		self.assertEqual(
			path3, Path(self.temp_dir) / "team3" / "cancer" / "lstm" / "20250518"
		)

	def test_make_version_path_handles_string_path(self):
		"""Test that make_version_path works with string paths."""
		path = make_version_path(
			str(self.temp_dir), "team4", "alzheimer", "pubmed_bert"
		)

		expected_path = (
			Path(self.temp_dir) / "team4" / "alzheimer" / "pubmed_bert" / "20250518"
		)
		self.assertEqual(path, expected_path)
	...
	def test_make_version_path_skips_existing_suffix(self):
		...
		os.makedirs(
			os.path.join(self.temp_dir, "team5", "parkinsons", "lstm", "20250518")
		)

		# Create the _2 version directory
		os.makedirs(
			os.path.join(self.temp_dir, "team5", "parkinsons", "lstm", "20250518_2")
		)

		# Call make_version_path - should create _3
		path = make_version_path(self.temp_dir, "team5", "parkinsons", "lstm")
		expected_path = (
			Path(self.temp_dir) / "team5" / "parkinsons" / "lstm" / "20250518_3"
		)
		self.assertEqual(path, expected_path)
