...
class DatasetTestCase(TestCase):
	...
	def test_collect_articles_with_window(self):
		...
		for i in range(3, 5):
			self.articles[i].discovery_date = datetime.now() - timedelta(
				days=3, hours=i
			)
			self.articles[i].save()
	...
	def test_train_val_test_split_invalid_proportions(self):
		...
		for i in range(100):
			data.append(
				{"article_id": i, "text": f"This is article {i}", "relevant": i % 2}
			)
		df = pd.DataFrame(data)
	...
	def test_collect_articles_skips_null_relevance(self):
		...
		self.assertEqual(
			articles.count(), 5
		)  # Only the original 5, not the unreviewed one
	...
	def test_build_dataset_handles_null_relevance(self):
		...
		ArticleSubjectRelevance.objects.create(
			article=unreviewed_article, subject=self.subject, is_relevant=None
		)
