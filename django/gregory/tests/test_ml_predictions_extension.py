...
class MLPredictionsExtensionTestCase(TestCase):
	def setUp(self):
		...
		self.subject = Subject.objects.create(
			subject_name="Neurology", team=self.team, subject_slug="neurology"
		)
	...
	def test_relations(self):
		...
		self.assertEqual(
			self.article1.ml_predictions_detail.first(), self.ml_prediction
		)

		# Check related_name on Subject
		self.assertEqual(self.subject.ml_subject_predictions.count(), 1)
		self.assertEqual(
			self.subject.ml_subject_predictions.first(), self.ml_prediction
		)
	...
	def test_get_latest_prediction(self):
		...
		# Test getting the latest prediction for a specific model version
		latest_v1 = MLPredictions.get_latest_prediction(
			self.article1, self.subject, model_version="v1.0.0"
		)
		self.assertEqual(
			latest_v1, self.ml_prediction
		)  # Our original prediction from setUp

		# Test getting the latest prediction across all model versions
		latest_any_version = MLPredictions.get_latest_prediction(
			self.article1, self.subject
		)
		self.assertEqual(
			latest_any_version, newer_prediction
		)  # Should be the newest one

		# Test getting prediction for non-existent article/subject
		non_existent = MLPredictions.get_latest_prediction(
			self.article2, self.subject, model_version="v4.0.0"
		)
