...
from gregory.management.commands.import_articles_from_api import (
...
def _make_source(org, method="rss", name="Test Source"):
	team = Team.objects.create(
		organization=org,
		name="Test Team %s" % name,
		slug="test-team-%s" % name.lower().replace(" ", "-"),
	)
	subject = Subject.objects.create(
		subject_name="Subj %s" % name,
		subject_slug="subj-%s" % name.lower().replace(" ", "-"),
	)
