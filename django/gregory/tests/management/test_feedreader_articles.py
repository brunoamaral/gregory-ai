...
class TestFeedFetching(TestCase):
	...
	def test_fetch_feed_without_ssl_ignore(self, mock_feedparser):
		...
		result = self.command.fetch_feed(
			"https://example.com/feed.xml", ignore_ssl=False
		)
	...
	def test_fetch_feed_with_ssl_ignore(self, mock_feedparser, mock_requests):
		...
		result = self.command.fetch_feed(
			"https://example.com/feed.xml", ignore_ssl=True
		)

		mock_requests.get.assert_called_once_with(
			"https://example.com/feed.xml", verify=False, timeout=30
		)
...
class TestSummaryExtraction(TestCase):
	...
	def test_summary_extraction_priority_pubmed(self):
		...
		with patch("gregory.classes.SciencePaper.clean_abstract") as mock_clean:
			...
			if (
				"pubmed" in "https://pubmed.ncbi.nlm.nih.gov/rss/search"
				and "content" in entry
			):
				summary = entry["content"][0]["value"]

			feed_summary = (
				SciencePaper.clean_abstract(abstract=summary) if summary else ""
			)
...
class TestDOIExtraction(TestCase):
	...
	def test_doi_extraction_pubmed(self):
		...
		entry = {"dc_identifier": "doi:10.1234/example.doi", "title": "Test Article"}
		source_link = "https://pubmed.ncbi.nlm.nih.gov/rss/search"

		doi = None
		if "pubmed" in source_link and entry.get("dc_identifier", "").startswith(
			"doi:"
		):
	...
	def test_doi_extraction_faseb(self):
		...
		entry = {"prism_doi": "10.1096/example.doi", "title": "Test Article"}
...
class TestCrossRefIntegration(TestCase):
	...
	def test_crossref_failed_refresh(self, mock_clean):
		...
		if isinstance(refresh_result, str) and any(
...
class TestCrossRefDataUpdates(TransactionTestCase):
	...
	def test_existing_article_crossref_update_integration(
		self, mock_science_paper_class
	):
		...
		with patch(
			"gregory.management.commands.feedreader_articles.SciencePaper.clean_abstract",
			return_value="Cleaned enhanced abstract",
		):
			command = Command()
			command.setup()
	...
	def test_authors_processed_when_crossref_updated(self, mock_science_paper_class):
		...
		jane_smith_db = Authors.objects.get(
			ORCID="https://orcid.org/0000-0000-0000-0002"
		)
	...
	def test_create_or_update_article_with_crossref_title_change(self):
		...
		updated_article, created, crossref_was_updated = (
			command.create_or_update_article(
				doi="10.1234/test.doi",
		...
		self.assertTrue(
			crossref_was_updated
		)  # Should be True since CrossRef data was added
...
class TestArticleCreationAndUpdate(TransactionTestCase):
	...
	def test_article_duplicate_detection_by_doi(self):
		...
		article1 = Articles.objects.create(
			doi=doi, title="First Title", link="https://example.com/first"
		)
		...
		existing_article = Articles.objects.filter(
			Q(doi=doi) | Q(title="Different Title")
		).first()
	...
	def test_article_duplicate_detection_by_title(self):
		...
		# Create first article
		article1 = Articles.objects.create(
			title=title, link="https://example.com/first"
		)
...
class TestAuthorProcessing(TransactionTestCase):
	...
	def test_author_creation_without_orcid(self):
		...
		author_info = {"given": "Jane", "family": "Smith"}
	...
	def test_author_duplicate_handling(self):
		...
		Authors.objects.create(given_name="John", family_name="Duplicate", ORCID=None)
		...
		try:
			author_obj, author_created = Authors.objects.get_or_create(
				given_name="John", family_name="Duplicate"
			)
			...
			author_obj = next(
				(author for author in authors if author.ORCID), authors.first()
			)
	...
	def test_skip_author_with_incomplete_data(self):
		...
		author_info_incomplete = {"given": None, "family": "OnlyFamily"}
...
class TestSummaryLengthWarnings(TestCase):
	...
	def test_summary_length_warning_detection(self):
		...
		test_cases = [
			...
			(
				"This is a summary that is between twenty and five hundred characters long.",
				True,
			),  # 20-500 range
	...
	def test_database_error_handler(self):
		...
		with patch.object(command, "stdout") as mock_stdout:
			...
			mock_stdout.write.assert_called_once_with(
				"An error occurred during test operation: Test error"
			)
...
class TestFeedreaderArticlesIntegration(TransactionTestCase):
	...
	def test_full_article_processing_with_doi(
		self, mock_remove_utm, mock_science_paper_class, mock_feedparser
	):
	...
	@patch("gregory.management.commands.feedreader_articles.feedparser")
	@patch("gregory.functions.remove_utm")
	def test_full_article_processing_without_doi(
		self, mock_remove_utm, mock_feedparser
	):
		...
		with patch.object(command, "fetch_feed", return_value=mock_feed):
			with patch(
				"gregory.classes.SciencePaper.clean_abstract",
				return_value="Cleaned summary",
			):
				command.update_articles_from_feeds()
...
class TestNatureFeedProcessor(TestCase):
	...
	def test_extract_doi_invalid_links(self):
		...
		invalid_cases = [
			...
			{
				"entry": {
					"link": "https://www.nature.com/no-articles-path",
					"title": "No articles path",
				}
			},
			{
				"entry": {
					"link": "https://www.nature.com/articles/",
					"title": "Empty articles path",
				}
			},
		]
	...
	def test_extract_summary(self):
		...
		test_cases = [
			{"entry": {"summary": "Test summary"}, "expected": "Test summary"},
	...
	def test_nature_feed_processor_integration(self):
		...
		nature_processors = [
			p for p in command.feed_processors if isinstance(p, NatureFeedProcessor)
		]
...
class TestSagePublicationsFeedProcessor(TestCase):
	...
	def test_extract_doi_from_prism_doi(self):
		...
		entry = {"prism_doi": "10.1177/21582440251334940", "title": "Test Article"}
	...
	def test_extract_doi_invalid_entries(self):
		...
		invalid_cases = [
			...
			{
				"entry": {"dc_identifier": "", "title": "Empty dc_identifier"}
			},  # Empty dc_identifier
			{
				"entry": {
					"link": "https://journals.sagepub.com/no-doi",
					"title": "No DOI in link",
				}
			},
			{"entry": {"link": "", "title": "Empty link"}},  # Empty link
		]
	...
	def test_extract_summary_fallback_fields(self):
		...
		test_cases = [
			...
			{"entry": {"summary": "Summary content"}, "expected": "Summary content"},
			{
	...
	def test_sage_feed_processor_integration(self):
		...
		sage_processors = [
			p
			for p in command.feed_processors
			if isinstance(p, SagePublicationsFeedProcessor)
		]
	...
	def test_keyword_filtering_inheritance(self):
		...
		self.assertFalse(
			self.processor.should_include_article(non_matching_entry, source)
		)
...
class TestCommandExecution(TestCase):
	...
	def test_command_help_text(self):
		...
		command = Command()
		self.assertEqual(
			command.help, "Fetches and updates articles and trials from RSS feeds."
		)
