...
class TestPredictArticlesCommand(TestCase):
	...
	@patch(
		"gregory.management.commands.predict_articles.Command.handle", return_value=None
	)
	...
	def test_invalid_args_team_and_all_teams(self):
		...
		"""
		with patch(
			"gregory.management.commands.predict_articles.Command.handle",
			return_value=None,
		) as mock_handle:
...
class TestGetArticles(TestCase):
	...
	def test_get_articles_basic_filtering(self):
		...
		self.assertEqual(
			articles.count(), 1, "Expected only one article related to the subject"
		)
		self.assertEqual(
			articles.first().title,
			"Article 1",
			"Expected article1 to be the only article related to the subject",
		)
		...
		def get_articles_decorator(
			subject, algorithm, model_version, lookback_days=None, all_articles=False
		):
			# Get the original queryset
			queryset = original_get_articles(
				subject, algorithm, model_version, lookback_days, all_articles
			)
		...
		# Apply the patch
		with patch(
			"gregory.management.commands.predict_articles.get_articles",
			get_articles_decorator,
		):
			# Get articles with 90-day lookback period
			articles = get_articles(
				self.subject, self.algorithm, self.model_version, 90
			)
			...
			self.assertEqual(
				articles.count(),
				1,
				"Expected only one article within the 90-day window",
			)
			self.assertEqual(
				articles.first().title,
				"Article 1",
				"Expected article1 to be the only article within the 90-day window",
			)
	...
	def test_get_articles_excludes_empty_summaries(self):
		...
		article2 = Articles.objects.create(
			title="Article 2", link="http://example.com/2", summary=""
		)
		article2.subjects.add(self.subject)

		article3 = Articles.objects.create(
			title="Article 3", link="http://example.com/3", summary=None
		)
	...
	def test_get_articles_with_different_lookback(self):
		...
		article1.discovery_date = timezone.make_aware(
			datetime.combine(today - timedelta(days=20), datetime.min.time())
		)
		article1.save()

		article2.discovery_date = timezone.make_aware(
			datetime.combine(today - timedelta(days=40), datetime.min.time())
		)
		article2.save()

		# Test with 30 days lookback
		articles = get_articles(
			self.subject, self.algorithm, self.model_version, lookback_days=30
		)
		...
		articles = get_articles(
			self.subject, self.algorithm, self.model_version, lookback_days=60
		)
...
class TestResolveModelVersion(TestCase):
	...
	def test_resolve_model_version_no_versions(self):
		...
		with self.assertRaises(FileNotFoundError):
			resolve_model_version(empty.name)
		empty.cleanup()
...
class TestBulkCreateWithDuplicateHandling(TestCase):
	...
	def test_bulk_create_ignore_conflicts(self):
		...
		self.assertEqual(
			MLPredictions.objects.get(article=self.article1).probability_score, 0.95
		)
...
class TestRunPredictionsFor(TestCase):
	...
	def test_run_predictions_success(
		self,
		mock_prepare_text,
		mock_load_model,
	...
	def test_run_predictions_dry_run(
		...
		logs_before = PredictionRunLog.objects.count()
		stats = self.command.run_predictions_for(
