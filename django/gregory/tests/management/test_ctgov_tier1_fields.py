...
def _source():
	...
	team = Team.objects.create(
		organization=org, name="Test Team Tier1", slug="test-team-tier1"
	)
...
class Tier1CreateTest(TestCase):
	...
	def test_create_stores_study_design(self):
		self.cmd.create_new_trial(
			_trial(study_design="Allocation: RANDOMIZED"), self.source
		)
	...
	def test_create_stores_secondary_sponsor(self):
		self.cmd.create_new_trial(
			_trial(nct="NCT88880003", secondary_sponsor="Org A; Org B"), self.source
		)
	...
	def test_create_stores_last_refreshed_on(self):
		...
		self.cmd.create_new_trial(
			_trial(nct="NCT88880004", last_refreshed_on=d), self.source
		)
	...
	def test_create_stores_date_enrollement(self):
		...
		self.cmd.create_new_trial(
			_trial(nct="NCT88880005", date_enrollement=d), self.source
		)
	...
	def test_create_stores_contact_affiliation(self):
		self.cmd.create_new_trial(
			_trial(nct="NCT88880006", contact_affiliation="University Hospital"),
			self.source,
		)
...
class Tier1UpdateGuardTest(TestCase):
	...
	def test_update_replaces_study_design_with_new_value(self):
		...
		self.cmd.update_existing_trial(
			t, _trial(nct="NCT88881001", study_design="New design"), self.source
		)
		t.refresh_from_db()
	...
	def test_update_replaces_secondary_sponsor(self):
		...
		self.cmd.update_existing_trial(
			t, _trial(nct="NCT88881003", secondary_sponsor="New Org"), self.source
		)
		t.refresh_from_db()
	...
	def test_update_does_not_blank_secondary_sponsor(self):
		...
		self.cmd.update_existing_trial(
			t, _trial(nct="NCT88881004", secondary_sponsor=None), self.source
		)
		t.refresh_from_db()
	...
	def test_update_replaces_contact_affiliation(self):
		...
		self.cmd.update_existing_trial(
			t,
			_trial(nct="NCT88881005", contact_affiliation="New Hospital"),
			self.source,
		)
		t.refresh_from_db()
	...
	def test_update_replaces_last_refreshed_on(self):
		...
		self.cmd.update_existing_trial(
			t, _trial(nct="NCT88881007", last_refreshed_on=new_date), self.source
		)
		t.refresh_from_db()
	...
	def test_update_does_not_blank_last_refreshed_on(self):
		...
		self.cmd.update_existing_trial(
			t, _trial(nct="NCT88881008", last_refreshed_on=None), self.source
		)
		t.refresh_from_db()
		self.assertEqual(t.last_refreshed_on, d)
