...
class TestGetArticlesAllArticles(PredictArticlesTestMixin, TestCase):
	...
	def test_all_articles_ignores_date_filter(self):
		...
		articles = get_articles(
			self.subject, "pubmed_bert", "v1", lookback_days=30, all_articles=False
		)
		self.assertEqual(articles.count(), 0)

		# With all_articles=True, both should appear
		articles = get_articles(
			self.subject, "pubmed_bert", "v1", lookback_days=30, all_articles=True
		)
...
class TestRunPredictionsEdgeCases(PredictArticlesTestMixin, TestCase):
	...
	def test_skipped_when_prepare_text_returns_none(
		self, mock_prepare, mock_load, mock_resolve, mock_get
	):
	...
	def test_zero_articles_returns_empty_stats(
		self, mock_prepare, mock_load, mock_resolve, mock_get
	):
	...
	def test_prediction_values_stored_correctly(
		self, mock_prepare, mock_load, mock_resolve, mock_get
	):
...
class TestPredictionRunLogModelVersion(PredictArticlesTestMixin, TestCase):
	...
	def test_run_log_has_resolved_model_version(
		self, mock_prepare, mock_load, mock_resolve, mock_get
	):
	...
	def test_run_log_marked_successful_on_clean_run(
		self, mock_prepare, mock_load, mock_resolve, mock_get
	):
