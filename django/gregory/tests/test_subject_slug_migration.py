...
class SubjectSlugMigrationTestCase(TestCase):
	def create_subject_with_slug(self, name, team):
		...
		return Subject.objects.create(subject_name=name, team=team, subject_slug=slug)
	...
	def setUp(self):
		...
		self.subject3 = self.create_subject_with_slug(
			"Neurology", self.team2
		)  # Same name but different team
	...
	def test_slug_generation(self):
		...
		for subject in Subject.objects.all():
			...
			self.assertTrue(
				subject.subject_slug == base_slug
				or subject.subject_slug.startswith(f"{base_slug}-"),
