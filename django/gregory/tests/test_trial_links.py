...
from gregory.utils.registry_utils import registry_from_url, merge_links, canonical_link
...
class RegistryFromUrlTest(SimpleTestCase):
	def test_known_registries(self):
		...
		self.assertEqual(
			registry_from_url(
				"https://www.clinicaltrialsregister.eu/ctr-search/trial/2024-000001-11/GB"
			),
			"euctr",
		)
		self.assertEqual(
			registry_from_url("https://www.isrctn.com/ISRCTN12345678"), "isrctn"
		)

	def test_www_prefix_and_subdomains(self):
		self.assertEqual(
			registry_from_url("https://www.clinicaltrials.gov/study/NCT1"), "ctgov"
		)
		self.assertEqual(
			registry_from_url("https://beta.clinicaltrials.gov/study/NCT1"), "ctgov"
		)

	def test_unknown_domain_falls_back_to_hostname(self):
		self.assertEqual(
			registry_from_url("https://www.example-registry.org/trial/1"),
			"example-registry.org",
		)
...
class MergeTrialLinksTest(SimpleTestCase):
	...
	def test_never_overwrites_existing_entry(self):
		...
		merged = merge_links(
			existing, "https://clinicaltrials.gov/ct2/show/NCT00000001"
		)
	...
	def test_ignores_empty_url(self):
		self.assertEqual(
			merge_links({"ctgov": CTGOV_LINK}, None), {"ctgov": CTGOV_LINK}
		)
...
def _make_source(org, method="rss", name="Test Source"):
	team = Team.objects.create(
		organization=org, name="Test Team %s" % name, slug="test-team-%s" % method
	)
	subject = Subject.objects.create(
		subject_name="MS %s" % name, subject_slug="ms-%s" % method
	)
