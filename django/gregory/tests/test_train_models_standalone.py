...
class TrainModelsTest(unittest.TestCase):
	...
	def mock_trainer_factory(self, algorithm):
		...
		def mock_save(path):
			...
			return {"model_path": str(path), "weights_path": model_file}
		...
		try:
			...
			for algo in ["pubmed_bert", "lgbm_tfidf", "lstm"]:
				...
				model_dir = os.path.join(
					self.test_models_dir, "test-team", "test-subject", algo, version
				)
				...
				val_metrics = trainer.evaluate(
					test_texts=val_texts, test_labels=val_labels, threshold=0.8
				)

				test_metrics = trainer.evaluate(
					test_texts=test_texts, test_labels=test_labels, threshold=0.8
				)
