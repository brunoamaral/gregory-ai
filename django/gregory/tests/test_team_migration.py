...
class TeamMigrationTestCase(TestCase):
	def setUp(self):
		...
		self.org4 = Organization.objects.create(
			name="Test Team"
		)  # Duplicate name to test slug uniqueness
	...
	def test_team_slugs(self):
		...
		self.assertEqual(
			teams_with_null_slugs.count(), 0, "Found teams with NULL slugs"
		)
		...
		self.assertEqual(
			duplicate_orgs.count(),
			2,
			"Expected exactly 2 organizations with name 'Test Team'",
		)
		...
		self.assertEqual(
			len(duplicate_teams),
			2,
			"Expected exactly 2 teams for duplicate organizations",
		)
		...
		self.assertTrue(
			"test-team" in slugs
			and any(slug.startswith("test-team-") for slug in slugs),
