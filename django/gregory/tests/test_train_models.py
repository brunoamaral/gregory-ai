...
if (
	"DJANGO_SETTINGS_MODULE" not in os.environ
	or "test_settings" not in os.environ["DJANGO_SETTINGS_MODULE"]
):
...
class TrainModelsCommandTest(TestCase):
	...
	def test_train_models_command_execution(self):
		...
		try:
			...
			with patch(
				"gregory.management.commands.train_models.Command"
			) as MockCommandClass:
				...
				mock_command.run_training_pipeline.side_effect = (
					self._mock_run_training_pipeline
				)
	...
	def _mock_handle(self, *args, **options):
		...
		if (
			hasattr(self, "verboser")
			and options.get("verbose") == VerbosityLevel.SUMMARY
		):
	...
	def _mock_run_training_pipeline(self, team_slug, subject_slug, algorithm, options):
		...
		model_dir = os.path.join(
			self.test_models_dir, team_slug, subject_slug, algorithm, version
		)
	...
	def test_verbosity_levels(self):
		...
		try:
			...
			with patch(
				...
				self._run_command_test_with_verbosity(
					mock_command, VerbosityLevel.QUIET
				)
				...
				summary_output.write(
					"Team | Subject | Algorithm | Val Acc | Test Acc\n"
				)
				summary_output.write("--------------------------------------------\n")
				summary_output.write(
					"test-team | test-subject | pubmed_bert | 0.85 | 0.83\n"
				)
				...
				self._run_command_test_with_verbosity(
					mock_command, VerbosityLevel.SUMMARY
				)
				...
				for verbosity in ["quiet", "summary"]:
					...
					with open(
						os.path.join(model_dir, "pubmed_bert_model.bin"), "w"
					) as f:
						f.write("Mock model data")
				...
				self.assertLess(
					len(quiet_output.getvalue()), len(summary_output.getvalue())
				)
		except Exception as e:
			debug_print(f"Exception occurred: {e}")
			debug_print(traceback.format_exc())
	...
	def _run_command_test_with_verbosity(self, mock_command, verbosity_level):
		...
		options = {
			...
			"parsed_algos": [
				"pubmed_bert"
			],  # Just use one algorithm for the verbosity test
		}
