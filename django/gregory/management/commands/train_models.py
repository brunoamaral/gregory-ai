...
try:
	...
	warnings.warn(
		f"ML modules could not be imported: {str(e)}. Some features will be unavailable."
	)
	ML_AVAILABLE = False
...
class Command(BaseCommand):
	...
	def validate_arguments(self, options):
		...
		if options["subject"] and not options["team"]:
			raise CommandError(
				"--subject can only be used with --team, not with --all-teams"
			)
		...
		if not input_algos.issubset(valid_algos):
			...
			raise CommandError(
				f"Invalid algorithm(s): {', '.join(invalid_algos)}. "
	...
	def log_success(
		self, message: str, min_verbosity: VerbosityLevel = VerbosityLevel.PROGRESS
	):
	...
	def log_warning(
		self, message: str, min_verbosity: VerbosityLevel = VerbosityLevel.WARNINGS
	):
	...
	def get_teams_and_subjects(self, options) -> List[Tuple[str, List[str]]]:
		...
		if options["all_teams"]:
			...
			for team in teams:
				subject_slugs = list(
					team.subjects.values_list("subject_slug", flat=True)
				)
			...
			if options["subject"]:
				...
				try:
					subject = Subject.objects.get(
						team=team, subject_slug=options["subject"]
					)
				...
				if subject_slugs:
					...
					self.log_warning(
						f"Team '{team.slug}' has no subjects, skipping",
	...
	def run_training_pipeline(
		self, team_slug: str, subject_slug: str, algorithm: str, options: Dict[str, Any]
	) -> Dict[str, Any]:
		"""
		...
		if model_version:
			...
			model_dir = make_version_path(
				BASE_MODEL_DIR, team_slug, subject_slug, algorithm
			)
		...
		if len(dataset_df) == 0:
			raise ValueError(
				f"No labeled articles found for {team_slug}/{subject_slug}"
			)
		...
		if len(class_counts) < 2:
			raise ValueError(
				f"Dataset has only one class. Two classes (0 and 1) are required for training."
			)

		if min_class_count < 2:
			raise ValueError(
				f"The least populated class in y has only {min_class_count} member, which is too few. "
		...
		if len(class_counts) < 2:
			raise ValueError(
				f"Cannot split dataset: only one class present ({class_counts.to_dict()})"
			)

		min_class_count = class_counts.min()
		if min_class_count < 2:
			raise ValueError(
				f"Cannot split dataset: the minority class has only {min_class_count} samples. "
		...
		try:
			self.log_message(
				"Splitting dataset into train/val/test sets", VerbosityLevel.PROGRESS
			)
			self.log_message(
				f"Class distribution before split: {class_counts.to_dict()}",
				VerbosityLevel.WARNINGS,
			)
		...
		if options["pseudo_label"]:
			self.log_message(
				"Applying pseudo-labeling with self-training", VerbosityLevel.PROGRESS
			)
		...
		try:
			...
			val_metrics = trainer.evaluate(
				test_texts=val_texts, test_labels=val_labels, threshold=threshold
			)

			# Evaluate on test set
			test_metrics = trainer.evaluate(
				test_texts=test_texts, test_labels=test_labels, threshold=threshold
			)
			...
			self.log_error(
				f"Error during model evaluation: {str(e)}", VerbosityLevel.WARNINGS
			)
		...
		self.log_message(
			f"Saving model artifacts to {model_dir}", VerbosityLevel.PROGRESS
		)
		...
		self.log_message(
			"Checking metrics for JSON serializability...", VerbosityLevel.WARNINGS
		)
		for key, value in formatted_metrics.items():
			self.log_message(
				f"  {key}: {type(value).__name__} = {str(value)[:50]}",
				VerbosityLevel.WARNINGS,
			)
		...
		try:
			...
			self.log_error(
				f"JSON serialization error: {str(e)}", VerbosityLevel.WARNINGS
			)
			...
			for key, value in formatted_metrics.items():
				try:
					...
					self.log_warning(
						f"Skipping non-serializable metric: {key} (type: {type(value).__name__}, error: {te})",
						VerbosityLevel.WARNINGS,
					)
	...
	def handle(self, *args, **options):
		...
		if options.get("debug"):
			...
			try:
				...
				self.stdout.write(
					f"  Using GPU: {device_info.get('using_gpu', False)}\n"
				)
				self.stdout.write("\n")
			except Exception as e:
		...
		try:
					...
					for algo in algos:
						...
						try:
							...
							try:
								...
								self.log_error(
									f"Failed to update run log: {str(db_error)}",
									VerbosityLevel.WARNINGS,
								)
			...
			if all_results:
				...
				for result in all_results:
					...
					val_f1_fmt = (
						f"{val_f1:.4f}" if isinstance(val_f1, (int, float)) else val_f1
					)
					test_f1_fmt = (
						f"{test_f1:.4f}"
