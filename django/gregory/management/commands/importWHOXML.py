...
class Command(BaseCommand):
	...
	def update_existing_trial(self, trial, trial_data, source, subject):
			...
			# Handle other fields
				...
				if isinstance(current_value, dict) and isinstance(value, dict):
					...
					for k, v in value.items():
						if v and (
							k not in merged_identifiers or merged_identifiers[k] is None
						):
		...
		if has_changes:
			...
			self.stdout.write(
				f"Saving changes for trial: {trial.trial_id}. Changes: {updated_fields}"
			)
			trial.save()
	...
	def create_new_trial(self, trial_data, source, subject):
		try:
			...
			self.stdout.write(
				self.style.ERROR(
					f"Error creating trial: {trial_data.get('title', 'Unknown')}. Error: {e}"
				)
			)
			return None
	...
	def check_for_existing_trial(self, trial_data, source, subject):
		...
		if "identifiers" in trial_data and trial_data["identifiers"]:
			...
			trial_id_value = list(trial_data["identifiers"].values())[
				0
			]  # Extract the trialid value
			identifier_key = list(trial_data["identifiers"].keys())[
				0
			]  # Extract the identifier key
			...
			if identifier_key:
				...
				if existing_trial and identifiers_conflict(
					existing_trial.identifiers, trial_data.get("identifiers")
				):
					existing_trial = None
		...
		if not existing_trial and "title" in trial_data:
			...
			if candidate and not identifiers_conflict(
				candidate.identifiers, trial_data.get("identifiers")
			):
	...
	def parse_xml(self, xml_file_path, source_id):
		try:
			...
			self.stdout.write(
				self.style.ERROR(f"Source with ID {source_id} not found.")
			)
			return
		...
		if subject is None:
			self.stdout.write(
				self.style.ERROR(f"Source {source_id} has no associated subject.")
			)
			return
		...
		for trial in root.findall("Trial"):
			...
			if trial_identifier:
				...
				key = "".join(
					filter(str.isalpha, trial_identifier.split("-")[0])
				).lower()
				trial_data["identifiers"] = {key: trial_identifier}
			if not trial_identifier:
				self.stdout.write(
					self.style.WARNING(
						f"Missing or invalid TrialID for trial: {self.get_text(trial, 'Public_title')}. Skipping."
					)
				)
				continue
			...
			self.stdout.write(
				self.style.NOTICE(
					f"Processing trial: {trial_data['title']} with TrialID: {trial_data['identifiers']}"
				)
			)
