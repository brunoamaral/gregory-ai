...
class Command(BaseCommand):
	...
	def handle(self, *args, **options):
		...
		self.stdout.write(
			f"Clearing logs older than {days} days (before {cutoff_date.strftime('%Y-%m-%d %H:%M:%S')})"
		)
		self.stdout.write("")
		...
		if not prediction_logs_only:
			...
			if api_log_count > 0:
				if dry_run:
					self.stdout.write(
						f"API Access Logs: Would delete {api_log_count} log(s)"
					)
				else:
					deleted_count, _ = api_logs.delete()
					self.stdout.write(
						self.style.SUCCESS(
							f"API Access Logs: Deleted {deleted_count} log(s)"
						)
					)
		...
		if not api_logs_only:
			prediction_logs = PredictionRunLog.objects.filter(
				run_started__lt=cutoff_date
			)
			...
			if prediction_log_count > 0:
				if dry_run:
					self.stdout.write(
						f"Prediction Run Logs: Would delete {prediction_log_count} log(s)"
					)
				else:
					deleted_count, _ = prediction_logs.delete()
					self.stdout.write(
						self.style.SUCCESS(
							f"Prediction Run Logs: Deleted {deleted_count} log(s)"
						)
					)
					total_deleted += deleted_count
			else:
		...
		if dry_run:
			...
			self.stdout.write(
				self.style.SUCCESS(f"Total deleted: {total_deleted} log(s)")
			)
