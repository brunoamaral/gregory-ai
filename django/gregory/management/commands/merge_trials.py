...
from gregory.utils.registry_utils import canonical_link, merge_links
		...
		with transaction.atomic():
			for rid in remove_ids:
				try:
					...
					self.stderr.write(
						self.style.WARNING(f"skip: trial {rid} does not exist.")
					)
					continue
				...
				self.stdout.write(
					self.style.SUCCESS(f"   merged and deleted trial {rid}.")
				)
