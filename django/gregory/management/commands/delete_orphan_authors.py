...
class Command(BaseCommand):
	...
	def handle(self, *args, **options):
		...
		if count == 0:
			self.stdout.write(
				self.style.SUCCESS(
					"No orphan authors found. All authors have at least one article."
				)
			)
			return
		...
		for author in orphan_authors[:50]:  # Show first 50
			...
			self.stdout.write(
				f"  - ID: {author.author_id} | Name: {author.full_name or f'{author.given_name} {author.family_name}'}{orcid_info}"
			)
		...
		if dry_run:
			self.stdout.write(
				self.style.WARNING(
					f"DRY RUN: Would delete {count} author(s). No changes made."
				)
			)
			return
		...
		if not force:
			self.stdout.write(
				self.style.WARNING(f"This will permanently delete {count} author(s).")
			)
