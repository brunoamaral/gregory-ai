...
class Command(BaseCommand):
	def handle(self, *args, **options):
		...
		dataset = pd.DataFrame(list(Articles.objects.annotate(abstract_length=Length('summary')).filter(abstract_length__gte=25).filter(abstract_length__lte=2000).filter(kind='science paper').filter(takeaways=None)[:100].values("article_id", "summary",)))
