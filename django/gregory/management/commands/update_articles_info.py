...
class Command(BaseCommand):
	...
	def handle(self, *args, **kwargs):
		...
		self.stdout.write(
			self.style.SUCCESS(
				"Successfully updated articles information with minimal API calls."
			)
		)
	...
	def update_article_details(self):
		...
		self.stdout.write(
			f"Found {total_articles} articles to update (including {articles_with_summaries.count()} with truncated summaries)"
		)
	...
	def update_article_from_paper(self, article, paper):
		...
		should_update_summary = (
			article.summary is None
			or article.summary == "not available"
			or self.is_summary_truncated(article.summary)
		)
			...
			if paper.abstract:
				...
				if article.summary != paper.abstract:
					if self.is_summary_truncated(article.summary):
						self.stdout.write(
							f"Replacing truncated summary for '{article.title}'"
						)
						self.stdout.write(
							f"  Old (truncated): {article.summary[-50:] if article.summary else 'None'}"
						)
				...
				self.stdout.write(
					f"No abstract found for article '{article.title}' with DOI {article.doi}."
				)
