...
class Command(BaseCommand):
	...
	def handle(self, *args, **options):
		...
		if count == 0:
			self.stdout.write(
				self.style.SUCCESS(
					"No orphan articles found. All articles have at least one source."
				)
			)
			return
		...
		for article in orphan_articles[:50]:  # Show first 50
			self.stdout.write(
				f"  - ID: {article.article_id} | Title: {article.title[:80]}..."
				if len(article.title) > 80
				else f"  - ID: {article.article_id} | Title: {article.title}"
			)
		...
		if dry_run:
			self.stdout.write(
				self.style.WARNING(
					f"DRY RUN: Would delete {count} article(s). No changes made."
				)
			)
			return
		...
		if not force:
			self.stdout.write(
				self.style.WARNING(f"This will permanently delete {count} article(s).")
			)
		...
		self.stdout.write(
			self.style.SUCCESS(f"Successfully deleted {deleted_count} object(s).")
		)
