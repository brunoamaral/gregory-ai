...
class LSTMTrainer:
	...
	def train(
		...
		train_text_ds = tf.data.Dataset.from_tensor_slices(train_texts).batch(
			batch_size
		)
		...
		logging.info(
			f"Model created with vocabulary size: {len(self.vectorizer.get_vocabulary())}"
		)
		...
		logging.info(
			f"Training completed in {self.training_time:.2f} seconds ({self.epochs_trained} epochs)."
		)

		return history

	def evaluate(
		self, test_texts: List[str], test_labels: List[int], threshold: float = 0.8
	...
	def save(self, model_dir: Union[str, Path]) -> Dict[str, str]:
		...
		vectorizer_config = {
			...
			"index_word": {
				i: word for i, word in enumerate(self.vectorizer.get_vocabulary())
			},
		}
		...
		if self.history is not None:
			...
			for metric in self.metrics_names:
				if metric in self.history.history:
					history_metrics[f"final_{metric}"] = self.history.history[metric][
						-1
					]
					history_metrics[f"final_val_{metric}"] = self.history.history[
						f"val_{metric}"
					][-1]
	...
	def load(self, model_dir: Union[str, Path]) -> None:
		...
		lookup = tf.keras.layers.StringLookup(
			vocabulary=vocabulary, mask_token="", num_oov_indices=1, oov_token="[UNK]"
		)
