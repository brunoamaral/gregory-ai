...
class BertTrainer:
	...
	def _create_model(self) -> tf.keras.Model:
		...
		x = Dense(self.dense_units, activation="relu", kernel_regularizer=l2(0.01))(
			cls_token
		)
		x = Dropout(0.3)(x)
	...
	def save(
		self, model_dir: Union[str, Path], save_format: str = "h5"
	) -> Dict[str, Any]:
		"""
		...
		return {"weights_path": str(weights_path), "metrics_info": metrics_info}
	...
	def perform_pseudo_labeling(
		...
		while len(remaining_unlabeled) > 0 and iteration < max_iterations:
			...
			labeled_labels_onehot = to_categorical(
				current_labeled_labels, num_classes=2
			)
			...
			if len(confident_idx) == 0:
				logging.info(
					f"No confident predictions above threshold {confidence_threshold} in iteration {iteration}."
				)
		...
		logging.info(
			f"Original size: {len(labeled_texts)}, Added: {len(current_labeled_texts) - len(labeled_texts)}"
		)
