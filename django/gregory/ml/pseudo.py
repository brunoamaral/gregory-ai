...
def generate_pseudo_labels(
	...
	if len(class_counts) < 2 or min_class_count < 1:
		if verbose:
			logging.warning(
				f"Training data has insufficient examples in the minority class ({min_class_count})"
			)
			logging.warning(f"Class distribution: {class_counts.to_dict()}")
			logging.warning(
				"Skipping pseudo-labeling as it requires at least 2 examples of each class"
			)
	...
	if algorithm == "pubmed_bert":
		...
		model_params.setdefault(
			"freeze_weights", True
		)  # Freeze the base model for efficiency
	...
	if verbose:
		logging.info(
			f"Starting pseudo-labeling with {len(train_df)} labelled and {len(unlabelled_df)} unlabelled examples"
		)
		logging.info(
			f"Using algorithm: {algorithm}, confidence threshold: {confidence}, max iterations: {max_iter}"
		)
	...
	while len(remaining_unlabelled) > 0 and iteration < max_iter:
		...
		confident_indices = [
			i for i, score in enumerate(confidence_scores) if score >= confidence
		]

		if not confident_indices:
			if verbose:
				logging.info(
					f"No confident predictions above threshold {confidence} in iteration {iteration}."
				)
		...
		remaining_indices = [
			i for i in range(len(remaining_unlabelled)) if i not in confident_indices
		]
		remaining_unlabelled = remaining_unlabelled.iloc[remaining_indices].reset_index(
			drop=True
		)

		if verbose:
			logging.info(
				f"Added {len(confident_indices)} pseudo-labelled examples with confidence >= {confidence}"
			)

	if verbose:
		logging.info(f"\nPseudo-labeling complete after {iteration} iterations")
		logging.info(
			f"Final training set: {len(train_df)} examples "
...
def get_pseudo_label_stats(
	df: pd.DataFrame, label_column: str = "relevant"
) -> Dict[str, Any]:
	"""
	...
	if not pseudo_df.empty:
		...
		if "pseudo_iteration" in pseudo_df.columns:
			iteration_counts = (
				pseudo_df["pseudo_iteration"].value_counts().sort_index().to_dict()
			)
