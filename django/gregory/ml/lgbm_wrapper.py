...
class LGBMTfidfTrainer:
	...
	def train(
		...
		**kwargs,  # Accept any additional kwargs for compatibility
		...
		try:
			...
			if (
				evals_result
				and "valid" in evals_result
				and "binary_logloss" in evals_result["valid"]
			):
				history = {
					"iterations": list(
						range(len(evals_result["valid"]["binary_logloss"]))
					),
	...
	def save(self, model_dir: Union[str, Path]) -> Dict[str, str]:
		...
		metrics_info = {
			...
			"lgbm_params": {
				k: v for k, v in self.lgbm_params.items() if not callable(v)
			},
