		...
		if gpus:
			for gpu in gpus:
				try:
					...
					if "must be set before GPUs have been initialized" in str(
						e
					) or "virtual devices configured" in str(e):
...
def disable_gpu():
	...
	try:
		...
		logging.warning(
			"Could not enable GPU in TensorFlow. Ensure this is called before any TensorFlow operations."
		)
		pass
