...
def collect_articles(
	team_slug: str, subject_slug: str, window_days: Optional[int] = None
) -> QuerySet:
	"""
...
def train_val_test_split(
	...
	if num_classes < 2:
		raise ValueError(
			f"Dataset has only one class: {class_counts.to_dict()}. "
	...
	if min_class_count < 3:
		logging.warning(
			f"Class with only {min_class_count} examples detected. Using non-stratified splitting."
		)
	...
	train_df, temp_df = train_test_split(
		df, train_size=train_size, stratify=df[stratify_col], random_state=random_state
	)
