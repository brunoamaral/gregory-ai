...
def article_review_status_view(request):
	...
	if request.user.is_superuser:
		...
		subjects = (
			Subject.objects.filter(team__organization__id__in=user_orgs)
	...
	if request.method == "POST" and selected_subject_id:
		...
		if selected_articles and action:
			...
			if action == "mark_relevant":
				...
				messages.success(
					request,
					f"{len(selected_articles)} articles marked as relevant for {subject.subject_name}",
				)
				...
				messages.success(
					request,
					f"{len(selected_articles)} articles marked as not relevant for {subject.subject_name}",
				)
				...
				messages.success(
					request,
					f"{len(selected_articles)} articles marked for review for {subject.subject_name}",
				)
	...
	for article in page_obj.object_list:
		...
		if is_relevant is True:
			status_html = mark_safe(
				'<span style="color: #009900; font-weight: bold;">Relevant</span>'
			)
		elif is_relevant is False:
			status_html = mark_safe(
				'<span style="color: #cc0000; font-weight: bold;">Not Relevant</span>'
			)
		else:
			status_html = mark_safe(
				'<span style="color: #f90; font-weight: bold;">Not Reviewed</span>'
			)
...
def mark_articles_as_relevant(article_ids, subject, is_relevant):
	...
	for article_id in article_ids:
		...
		relevance, created = ArticleSubjectRelevance.objects.get_or_create(
			article=article, subject=subject, defaults={"is_relevant": is_relevant}
		)
...
def update_article_relevance_ajax(request):
	...
	if request.method == "POST":
		try:
			...
			action = data.get(
				"action"
			)  # 'mark_relevant', 'mark_not_relevant', or 'mark_for_review'
			...
			if action == "mark_relevant":
				relevance, created = ArticleSubjectRelevance.objects.get_or_create(
					article=article, subject=subject, defaults={"is_relevant": True}
				)
				if not created:
				...
				relevance, created = ArticleSubjectRelevance.objects.get_or_create(
					article=article, subject=subject, defaults={"is_relevant": False}
				)
				if not created:
...
def source_detail_view(request, source_id):
	...
	if source.source_for == "trials":
		...
		last_date_val = (
			...
			.values_list("discovery_date", flat=True)
			.first()
		)
		last_date = last_date_val.date() if last_date_val else None
		recent_items = list(
			source.trials_set.order_by("-discovery_date").values(
				"trial_id", "title", "discovery_date"
			)[:10]
		)
		item_type = "trials"
	else:
		...
		last_date = last_date_val.date() if last_date_val else None
		recent_items = list(
			source.articles_set.order_by("-discovery_date").values(
				"article_id", "title", "discovery_date"
			)[:10]
		)
		item_type = "articles"
...
def source_activity_json(request, source_id):
	...
	if source.source_for == "trials":
		rows = (
			source.trials_set.filter(
				discovery_date__date__gte=start, discovery_date__date__lte=today
			)
...
def sources_overview_view(request):
		...
		if selected_ids and action in ("activate", "deactivate", "reassign_to_team"):
			...
			if action == "activate":
				...
				if len(org_ids) != 1:
					messages.error(
						request,
						"Selected sources span multiple organisations. Please select sources from a single organisation.",
					)
				else:
					...
					if form.is_valid():
						...
						if to_team.organization_id == org_ids[0]:
							...
							messages.success(
								request, f'{count} source(s) reassigned to "{to_team}".'
							)
						else:
							messages.error(
								request,
								"Target team must belong to the same organisation.",
							)
					else:
	...
	if f_q:
		qs = qs.filter(
			...
			| Q(link__icontains=f_q)
			| Q(description__icontains=f_q)
			| Q(keyword_filter__icontains=f_q)
		)
...
def subject_analytics_orgs(request):
	...
	return JsonResponse(
		{"organisations": [{"id": o.id, "name": o.name} for o in qs.order_by("name")]}
	)
...
def subject_analytics_teams(request):
	...
	return JsonResponse(
		{"teams": [{"id": t.id, "name": str(t)} for t in qs.order_by("name")]}
	)
	...
	return JsonResponse(
		{
			"subjects": [
				...
				for s in qs.order_by("subject_name")
			]
		}
	)
