...
class RebuildCatsArticles(CronJobBase):
	RUN_EVERY_MINS = 30
	schedule = Schedule(run_every_mins=RUN_EVERY_MINS)
	code = 'db_maintenance.rebuild_categories_articles'
	def do(self):
		load_dotenv()
		## GET ENV
...
class RebuildCatsTrials(CronJobBase):
	RUN_EVERY_MINS = 30
	schedule = Schedule(run_every_mins=RUN_EVERY_MINS)
	code = 'db_maintenance.rebuild_categories_trials'
	def do(self):
		load_dotenv()
		## GET ENV
		db_host = os.getenv('DB_HOST')
		...
		for cat in categories:
			...
			SELECT "trials"."trial_id", "categories"."category_id" FROM "categories" INNER JOIN "trials" ON lower("trials"."title") similar to %s AND "categories"."category_id" = %s 
			""", (terms,cat_id))
