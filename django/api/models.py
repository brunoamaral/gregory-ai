...
class APIAccessScheme(models.Model):
	...
	api_key = models.CharField(
		max_length=64, unique=True, blank=False, default=generateRandomAPIKey
	)

	# A short name to identify the client
	client_name = models.CharField(
		max_length=200, unique=False, blank=False, default="Client Name"
	)
	...
	client_contacts = models.CharField(
		max_length=200, unique=False, blank=False, default="client@contact.com"
	)
	...
	max_calls_minute = models.IntegerField(
		default=DEFAULT_MAX_CALLS_MINUTE, blank=False
	)
...
class APIAccessSchemeLog(models.Model):
	...
	api_access_scheme = models.ForeignKey(
		APIAccessScheme, on_delete=models.CASCADE, blank=True, null=True
	)
	...
	def __str__(self):
		return (
			"["
			+ self.access_date.strftime("%Y-%m-%d %H:%M:%S")
			+ "] "
			+ self.call_type
			+ " (from "
			+ self.ip_addr
