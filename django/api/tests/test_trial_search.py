...
class TrialSearchViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.organization = Organization.objects.create(name="Test Organization", slug="trial-search-org")
        OrganizationApiSettings.objects.filter(organization=self.organization).update(make_api_public=True)
        self.team = Team.objects.create(name="Test Team", slug="trial-search-team", organization=self.organization)
