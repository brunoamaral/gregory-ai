...
class ArticleAuthorFilterTests(TestCase):
	...
	def setUp(self):
		...
		self.organization = Organization.objects.create(
			name="Test Organization", slug="art-auth-filter-org"
		)
		...
		self.author2 = Authors.objects.create(
			family_name="Doe", given_name="Jane", full_name="Jane Doe"
		)
	...
	def test_combine_author_filter_with_other_filters(self):
		...
		response = self.client.get(
			f"/articles/?author_id={self.author1.author_id}&team_id={self.team.id}"
		)
		self.assertEqual(response.status_code, status.HTTP_200_OK)
	...
	def test_author_filter_backwards_compatibility(self):
		...
		response = self.client.get(
			f"/articles/?author_id={self.author1.author_id}&format=json&page=1"
		)
		self.assertEqual(response.status_code, status.HTTP_200_OK)
