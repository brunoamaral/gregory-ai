			...
			with self.subTest(last_days=invalid_value):
				try:
					...
					response = self.client.get(
						"/articles/", {"last_days": invalid_value}
					)
					...
					logging.info(
						f"✓ last_days='{invalid_value}': Status {response.status_code} (OK)"
					)
		...
		for valid_value in valid_values:
			with self.subTest(last_days=valid_value):
				try:
					...
					logging.info(
						f"✓ last_days='{valid_value}': Status {response.status_code} (OK)"
					)

				except json.JSONDecodeError:
					self.fail(
						f"✗ API returned invalid JSON for last_days='{valid_value}'"
					)
				except Exception as e:
