	...
	def test_renderer_returns_csv_bytes(self):
		...
		renderer_context = {
			"request": type(
				"obj",
				(object,),
				{"path": "/articles/", "query_params": {"format": "csv"}},
			)
		}
		...
		response = renderer.render(
			data, accepted_media_type="text/csv", renderer_context=renderer_context
		)
		...
		self.assertTrue(
			"Summary 2 with different line breaks" in rows[2][summary_index]
		)
