...
class AuthorSerializerTest(TestCase):
	...
	def setUp(self):
		...
		self.author_no_country = Authors.objects.create(
			given_name="Jane", family_name="Smith", ORCID="0000-0002-3456-7890"
		)
	...
	def test_author_serializer_articles_list_url(self):
		...
		expected_url = (
			f"https://api.example.com/articles/?author_id={self.author.author_id}"
		)
	...
	def test_article_author_serializer_basic_fields(self):
		...
		for field in expected_fields:
			self.assertIn(
				field, data, f"Field '{field}' missing from ArticleAuthorSerializer"
			)
	...
	def test_article_author_serializer_excludes_articles_fields(self):
		...
		for field in excluded_fields:
			self.assertNotIn(
				field, data, f"Field '{field}' should not be in ArticleAuthorSerializer"
			)
