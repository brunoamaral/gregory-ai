...
class CountCacheSmokeTest(TestCase):
	...
	def _cache_queries(self, captured):
		return [q["sql"] for q in captured if "gregory_cache" in q["sql"]]
	...
	def test_second_request_skips_real_count_query(self):
		...
		self.assertTrue(
			self._cache_queries(ctx.captured_queries),
			"expected at least one cache lookup query",
		)
