...
class ArticleSearchViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.organization = Organization.objects.create(name="Test Organization", slug="art-search-org")
        OrganizationApiSettings.objects.filter(organization=self.organization).update(make_api_public=True)
        self.team = Team.objects.create(name="Test Team", slug="art-search-team", organization=self.organization)
