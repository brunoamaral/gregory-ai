...
class PostArticleOrgScopingBase(TestCase):
	def setUp(self):
		...
		self.other_source = _make_source(
			self.other_team, self.other_subj, "pa-other-source"
		)
...
class CrossOrgPayloadTest(PostArticleOrgScopingBase):
	...
	def test_kind_mismatch_returns_400(self):
		...
		trials_source = _make_source(
			self.my_team, self.my_subj, "pa-trials-source", source_for="trials"
		)
		payload = {
