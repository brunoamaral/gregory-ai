...
class OptimizedCategorySerializer(CategorySerializer):
	...
	def get_authors_count(self, obj):
		...
		return (
			Authors.objects.filter(articles__article_id__in=Subquery(articles_subquery))
			.distinct()
			.count()
		)
	...
	def get_top_authors(self, obj):
		...
		top_authors = (
			Authors.objects.filter(articles__article_id__in=Subquery(articles_subquery))
			.annotate(category_articles_count=Count("articles", distinct=True))
...
class OptimizedMonthlyCountsViewSet(viewsets.ModelViewSet):
	...
	def get_monthly_counts(self, team_category, ml_threshold=0.5):
		...
		monthly_articles = (
			articles.annotate(month=TruncMonth("published_date"))
			.values("month")
		...
		monthly_trials = (
			trials.annotate(month=TruncMonth("published_date"))
			.values("month")
		...
		available_models = (
			MLPredictions.objects.filter(article__team_categories=team_category)
		...
		for model in available_models:
			...
			ml_articles = (
				Articles.objects.filter(
					article_id__in=article_ids_with_ml, team_categories=team_category
				)
...
class CategoryOptimizationMixin:
	...
	def get_category_stats_optimized(self, category_id):
		...
		author_count = Authors.objects.filter(
			Exists(
				Articles.objects.filter(
					team_categories=category, authors=OuterRef("pk")
				)
			)
		).count()
