...
def prepare_weekly_summary_context(articles, trials, subscriber, site, customsettings):
	...
	base_context.update(
		{
			"subscriber": subscriber,
			"title": customsettings.get("title", "Gregory AI")
			if isinstance(customsettings, dict)
			else getattr(customsettings, "title", "Gregory AI"),
...
def get_optimized_context_for_management_commands(
	email_type,
	articles=None,
	trials=None,
	subscriber=None,
...
def organize_content_for_email(
	articles=None, trials=None, email_type="weekly_summary", subscriber=None
):
	"""
	...
	if articles is not None:
		result["articles"] = organizer.organize_articles(
			articles, email_type, subscriber
		)
...
def prepare_personalized_greeting(subscriber, email_type="weekly_summary"):
	...
	return {
		...
		"is_personalized": hasattr(subscriber, "first_name")
		and bool(subscriber.first_name),
	}
