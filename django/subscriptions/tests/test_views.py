...
class FindSiteByDomainTest(TestCase):
	...
	def test_unknown_domain_returns_none(self):
		site = _find_site_by_domain('evil.com')
		self.assertIsNone(site)

	def test_unknown_subdomain_returns_none(self):
		site = _find_site_by_domain('api.evil.com')
		self.assertIsNone(site)
...
class ResolveSiteFromRequestTest(TestCase):
	...
	def test_origin_takes_precedence_over_referer(self):
		# Origin points to the known site; Referer points to something unknown.
	...
	def test_falls_back_to_host_header(self):
		# RequestFactory sets SERVER_NAME to 'testserver' by default; override it.
		request = self.factory.post('/', SERVER_NAME='example.com', SERVER_PORT='80')
		self.assertEqual(_resolve_site_from_request(request).domain, 'example.com')

	def test_unknown_origin_falls_back_to_default_site(self):
		# Origin points to an unregistered domain; Host is 'testserver' (also
		# unregistered). Both fall through to Site.objects.get_current().
		request = self.factory.post('/', HTTP_ORIGIN='https://unknown.invalid')
		# Falls back to Site.objects.get_current() which is SITE_ID=1 → example.com
		self.assertEqual(_resolve_site_from_request(request).domain, 'example.com')
...
class OriginValidationTest(TestCase):
	...
	def _post(self, list_obj, email, origin=None):
		headers = {}
	...
	def test_malicious_origin_rejected_with_403(self):
		...
		self.assertTrue(
			any('unauthorized origin' in msg for msg in log.output),
			msg=f"Expected 'unauthorized origin' warning, got: {log.output}",
		)
	...
	def test_no_origin_header_allowed_through_with_warning(self):
		...
		self.assertNotEqual(response.status_code, 403)
		self.assertTrue(
			any('no Origin or Referer header' in msg for msg in log.output),
			msg=f"Expected missing-header warning, got: {log.output}",
		)
	...
	def test_site_profile_recorded_from_origin_not_host(self):
		"""SubscriberSiteProfile.site should match the Origin header domain, not the API host."""
