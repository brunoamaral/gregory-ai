...
class TestLatestResearchCategories(TestCase):
	def setUp(self):
		...
		self.subject1 = Subject.objects.create(
			subject_name="Subject 1", team=self.team, subject_slug="subject-1"
		)
		self.subject2 = Subject.objects.create(
			subject_name="Subject 2", team=self.team, subject_slug="subject-2"
		)
		self.subject3 = Subject.objects.create(
			subject_name="Subject 3", team=self.team, subject_slug="subject-3"
		)

		# Create team categories
		self.category1 = TeamCategory.objects.create(
			team=self.team, category_name="Category 1", category_slug="category-1"
		)
		self.category1.subjects.add(self.subject1)

		self.category2 = TeamCategory.objects.create(
			team=self.team, category_name="Category 2", category_slug="category-2"
		)
		...
		self.test_list = Lists.objects.create(
			list_name="Test List", weekly_digest=True, team=self.team
		)
		...
		self.empty_list = Lists.objects.create(
			list_name="Empty List", weekly_digest=True, team=self.team
		)
	...
	def test_get_latest_research_by_category(self):
		...
		self.assertEqual(
			len(result[self.category1]), 2
		)  # Should have 2 articles assigned to category1
		self.assertEqual(
			len(result[self.category2]), 2
		)  # Should have 2 articles assigned to category2
