...
class SubscriberAdminActionTests(TestCase):
	def setUp(self):
		...
		self.ls_a = ListSubscription.objects.create(
			subscriber=self.sub, list=self.list_a, is_active=True
		)
		self.ls_b = ListSubscription.objects.create(
			subscriber=self.sub, list=self.list_b, is_active=True
		)
	...
	def test_make_inactive_only_affects_selected_subscribers(self):
		...
		other_ls = ListSubscription.objects.create(
			subscriber=other, list=self.list_a, is_active=True
		)
