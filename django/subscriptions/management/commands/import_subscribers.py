...
from subscriptions.models import Lists, Subscribers, ListSubscription, SubscriberSiteProfile
...
class Command(BaseCommand):
	...
	def _prompt_profile(self, email):
		...
		while True:
			try:
				...
				pass
			self.stdout.write(self.style.WARNING('  Invalid selection, please enter a number from the list above.'))
	...
	def handle(self, *args, **options):
		...
		try:
			subscription_list = Lists.objects.select_related('site').get(list_id=list_id)
		except Lists.DoesNotExist:
			raise CommandError(f'List with ID {list_id} does not exist.')

		site = subscription_list.site
		self.stdout.write(
			f'Importing to list "{subscription_list.list_name}" '
			f'(site: {site.domain})'
		)
		...
		for row in rows:
			...
			try:
				...
				self.stdout.write(
					self.style.ERROR(f'  ERROR – could not create subscriber {email}: {exc}')
				)
				errors += 1
				continue
