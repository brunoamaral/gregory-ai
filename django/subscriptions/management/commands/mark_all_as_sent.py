...
class Command(BaseCommand):
	...
	def handle(self, *args, **options):
		subscribers = Subscribers.objects.filter(active=True).prefetch_related(
			"subscriptions"
		)
		...
		for subscriber in subscribers:
			...
			if not lists_subscribed.exists():
				self.stdout.write(
					self.style.WARNING(
						f"Subscriber {subscriber.email} has no subscriptions."
					)
				)
				continue
			...
			for lst in lists_subscribed:
				...
				for article in articles:
					_, created = SentArticleNotification.objects.get_or_create(
						article=article, list=lst, subscriber=subscriber
					)
					if created:
						self.stdout.write(
							self.style.SUCCESS(
		...
		self.stdout.write(
			self.style.SUCCESS(
				"All articles and trials have been marked as sent to all subscribers."
			)
		)
