...
class Command(BaseCommand):
	...
	def add_arguments(self, parser):
		...
		parser.add_argument(
			"--trials-only", action="store_true", help="Only prune trial notifications"
		)
	...
	def handle(self, *args, **options):
		...
		self.stdout.write(
			f"Pruning notification records older than {days} days (before {cutoff_date.date()})"
		)
		...
		if not trials_only:
			article_notifs = SentArticleNotification.objects.filter(
			...
			if dry_run:
				self.stdout.write(
					f"  Would delete {article_count:,} article notification records"
				)
			else:
				# Delete in batches to avoid memory issues with large datasets
				deleted_articles = self._batch_delete(
					article_notifs, "article notifications"
				)
		...
		if not articles_only:
			...
			if dry_run:
				self.stdout.write(
					f"  Would delete {trial_count:,} trial notification records"
				)
			else:
		...
		if dry_run:
			self.stdout.write(
				self.style.SUCCESS("Dry run complete. No records were deleted.")
			)
		else:
			self.stdout.write(
				self.style.SUCCESS(
					f"Successfully deleted {total_deleted:,} old notification records"
				)
			)
	...
	def _batch_delete(self, queryset, description, batch_size=10000):
		...
		while True:
			...
			if deleted_count % 50000 == 0:
				self.stdout.write(
					f"    Deleted {deleted_count:,} / {total_count:,} {description}"
				)
