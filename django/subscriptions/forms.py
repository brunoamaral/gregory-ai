...
class ListsAdminForm(ModelForm):
	ml_threshold = forms.FloatField(
		...
		widget=forms.NumberInput(
			attrs={"step": "0.01", "min": "0.0", "max": "1.0", "style": "width: 100px;"}
		),
...
class AnnouncementAdminForm(ModelForm):
	def __init__(self, *args, **kwargs):
		...
		if user.is_superuser:
			...
			membership = user.organizations_organizationuser.order_by("pk").first()
			default_org = membership.organization if membership else org_qs.first()
		else:
				...
				raise forms.ValidationError(
					{
						"lists": (
							...
							"announcement. Offending lists: " + ", ".join(offenders)
						),
					}
				)
		return cleaned
