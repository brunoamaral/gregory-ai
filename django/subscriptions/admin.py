	...
	fieldsets = [
		...
		(
			"Content Settings",
			{
				...
				"and article age are each checked against their own registration/publication date, not when "
				"GregoryAI discovered them, so a bulk import of old content can't flood a newsletter. The ML "
				"threshold determines the minimum confidence level required for ML predictions to be considered "
				'relevant. When sort order is set to "Date", the ML threshold is not used for article selection.',
			},
		),
		(
			"Main Content",
