...
if args.load_from_csv == True:  # load data from csv file into a pandas dataframe
	...
	if not total_articles:
		...
		with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
			futures = {
				executor.submit(fetch_articles_page, get_api_url, api_key, page): page
...
articles_df["terms"] = (
	articles_df["title"].astype(str) + " " + articles_df["summary"].astype(str)
)
...
for model, pipeline in pipelines.items():
	...
	logging.info(
		" => Accuracy for the " + model + " model: {:2.1f}%".format(accuracy * 100)
	)
